# run_within_modality_mvpa_raw_logo.py
...
# PARAMETERS
VALID_EMOTIONS = [
    "anger", "happiness" #"anxiety", "disgust", ,
    #"interest", "neutral", "pride", "relief", "sadness"
] # adapt if I want to include/exclude certain emotions (must match labels in my data!)

VALID_MODALITIES = ["video", "audio", "audiovisual"]   # adapt if running on each modality separately
...
def make_subject_mask(sub_path, output_name="subject_mask.nii.gz"):
    ...
    mask_files = glob.glob(
        os.path.join(sub_path, "ses-*", "BAS2", "**", "mask.nii*"),
        recursive=True
    )
    if not mask_files:
...
# 2) LOAD BETAS + LABELS
def load_betas_and_labels(sub_path, task="recog"):
    """
    Load β-maps and parse labels.
    Assumes label format: run_task_emotion_modality (at least 4 fields)
    """
    all_rows = []
    pattern = os.path.join(sub_path, "ses-*", "BAS2", "**", "regressor_labels.csv")
    ...
    if not all_rows:
        raise FileNotFoundError(f"No valid β-maps found for {sub_path}")

    merged = pd.concat(all_rows, ignore_index=True)

    print(f"Loaded {len(merged)} β-maps for {os.path.basename(sub_path)}")
...
def extract_features_raw(df, mask_path, mask_cache="./mask_cache"):
    """
    Convert β-maps into voxel feature matrix WITHOUT standardization.
    (raw beta values as features)
    """
    masker = NiftiMasker(
        mask_img=mask_path,
        standardize=False,      # <-- RAW betas (NO z-scoring)
        memory=mask_cache,
        dtype="float32",
        verbose=0,
    )
    print(f"Extracting voxel patterns from {len(df)} β-maps...")
    start = time.time()
...
def run_logo_within_modality(
    ...
    # LOGO folds = unique sessions present in this modality
    ...
    ConfusionMatrixDisplay(cm, display_labels=le.classes_).plot(cmap="Blues", ax=plt.gca(), colorbar=False)
...
# 5) DESCRIPTIVE WEIGHT MAPS (OPTIONAL)
def save_descriptive_weight_map(masker, fitted_clf, out_png, out_nii=None, title=None, top_percent=1.0):
    """
    Fit classifier on ALL samples for a modality, then plot weights descriptively.
    For multiclass LinearSVC: take mean(abs(coef)) across classes.
    """
    ...
    # percentile threshold for visualization (keep strongest weights)
    if top_percent is not None and 0 < top_percent < 100:
        thr = np.percentile(np.abs(w), 100 - top_percent)
        w_vis = w.copy()
        w_vis[np.abs(w_vis) < thr] = 0.0
    else:
        w_vis = w

    weight_img = masker.inverse_transform(w_vis)
    ...
    display = plotting.plot_stat_map(
        weight_img, bg_img=bg, title=title or "SVM weights",
        display_mode="ortho"
    )
...
# RUNNER (THIS IS THE "MAIN" PART)

def main():
    # adapt paths and parameters as needed
    sub_path = r"D:\singleN_betas\sub-003"   # adapt this when running on sub-003 separately
    base_out_dir = r"C:\Users\Fabian\OneDrive - Stockholm University\Desktop\Eli\Master thesis"
    out_dir = os.path.join(base_out_dir, os.path.basename(sub_path), "mvpa_within_modality_happiness_anger")

    task = "recog"

    # If you want weight maps saved:
    MAKE_WEIGHT_MAPS = True
    WEIGHT_TOP_PERCENT = 1.0  # show top 1% absolute weights (visualization threshold)
    ...
    for m in VALID_MODALITIES:
        ...
        # Weight maps (fit on all samples of this modality, descriptive)
        if MAKE_WEIGHT_MAPS:
            Xk, yk_enc, gk = packed
            clf_full = LinearSVC(
                penalty="l2",
            ...
            wm_png = os.path.join(out_dir, f"{title_prefix}_{m}_weightmap_top{WEIGHT_TOP_PERCENT}pct.png")
            wm_nii = os.path.join(out_dir, f"{title_prefix}_{m}_weightmap.nii.gz")
            save_descriptive_weight_map(
                masker=masker,
                fitted_clf=clf_full,
                out_png=wm_png,
                out_nii=wm_nii,
                title=f"{title_prefix} | {m} | weights (top {WEIGHT_TOP_PERCENT}%)",
                top_percent=WEIGHT_TOP_PERCENT
            )
            print(f"[{m}] Saved weight map: {wm_png}")
